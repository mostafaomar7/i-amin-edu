...
import { locale as english } from 'app/main/apps/portals/i18n/en';
import { locale as arabic } from 'app/main/apps/portals/i18n/ar';
...
export class NewPortalComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._portalListService.uploadImage(imageFile).then((response: any) => {
  ...
  async getItem(id: string) {
    await this._portalListService.getItem(id).then((respone: any) => {
 ...
 */
  ...
  async saveItem() {
    await this._portalListService.addItem(this.gateForm.value).then(response => {
  ...
  async updateItem() {
    await this._portalListService.updateItem(this.gateForm.value).then(response => {
      this.isLoading = false;
      if (response.status) {
        this.back();
      } else {
