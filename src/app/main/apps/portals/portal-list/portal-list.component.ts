...
export class PortalListComponent implements OnInit {
   ...
   * @param {PortalListComponent} _portalListService
   */
  constructor(
 ...
 */
  addItem() {
    this.router.navigate([`/apps/portals/list/new`]);
  }

  editItem(row: any) {
    this.router.navigate([`/apps/portals/list/new/${row.id}`]);
  }

  deleteItem(id: string) {
    this.isLoading = true;
    this._portalListService.deleteItem(id).then((response: any) => {
      this.isLoading = false;
      if (response.status) {
        Swal.fire({
          icon: 'success',
