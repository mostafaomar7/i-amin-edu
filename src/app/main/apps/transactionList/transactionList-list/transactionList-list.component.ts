...
import { locale as english } from "app/main/apps/transactionList/i18n/en";
import { locale as arabic } from "app/main/apps/transactionList/i18n/ar";
import { CoreConfigService } from "@core/services/config.service";

@Component({
  selector: "app-transactionList-list",
  templateUrl: "./transactionList-list.component.html",
  styleUrls: ["./transactionList-list.component.scss"],
  encapsulation: ViewEncapsulation.None,
})
export class TransactionListListComponent implements OnInit {
   ...
   * @param {TransactionListListComponent} _transactionListListService
   */
  constructor(
    private _coreConfigService: CoreConfigService,
    private _transactionListListService: TransactionListListService,
   ...
   */
  async getRows() {
    await this._transactionListListService
      .getDataTableRows()
      .then((response) => {
  ...
  addItem() {
    this.router.navigate([`/apps/transaction-history/list/new`]);
  }

  editItem(row: any) {
    this.router.navigate([`/apps/transaction-history/list/new/${row.id}`]);
  }

  deleteItem(id: string) {
    this.isLoading = true;
    this._transactionListListService.deleteItem(id).then((response: any) => {
      this.isLoading = false;
      if (response.status) {
        Swal.fire({
          icon: "success",
