...
export class SubjectListComponent implements OnInit {
   ...
   * @param {SubjectListService} _subjectListService
   */
  constructor(
 ...
 */
  addItem() {
    this.router.navigate([`/apps/subjects/list/new`]);
  }

  editItem(row: any) {
    this.router.navigate([`/apps/subjects/list/new/${row.id}`]);
  }

  deleteItem(id: string) {
    this.isLoading = true;
    this._subjectListService.deleteItem(id).then((response: any) => {
      this.isLoading = false;
      if (response.status) {
        Swal.fire({
          icon: 'success',
