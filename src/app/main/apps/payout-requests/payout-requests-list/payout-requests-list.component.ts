...
@Component({
  ...
  templateUrl: './payout-requests-list.component.html',
  styleUrls: ['./payout-requests-list.component.scss'],
  encapsulation: ViewEncapsulation.None
})

export class PayoutRequestsListComponent implements OnInit {

  @Input() selectedCenterId: number;
 ...
 */
  async getRows() {
    await this.payoutRequestsListService.getDataTableRows().then(response => {
 ...
 */
  addItem() {
    this.router.navigate([`/apps/payout-requestss/list/new`]);
  }

  editItem(row: any) {
    this.router.navigate([`/apps/payout-requestss/list/new/${row.id}`]);
  }

  deleteItem(id: string) {
    this.isLoading = true;
    this.payoutRequestsListService.deleteItem(id).then((response: any) => {
      this.isLoading = false;
      if (response.status) {
        Swal.fire({
          icon: 'success',
