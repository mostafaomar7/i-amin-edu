...
export class StageListComponent implements OnInit {
   ...
   * @param {StageListService} _stageListService
   */
  constructor(
 ...
 */
  addItem() {
    this.router.navigate([`/apps/stages/list/new`]);
  }

  editItem(row: any) {
    this.router.navigate([`/apps/stages/list/new/${row.id}`]);
  }

  deleteItem(id: string) {
    this.isLoading = true;
    this._stageListService.deleteItem(id).then((response: any) => {
      this.isLoading = false;
      if (response.status) {
        Swal.fire({
          icon: 'success',
