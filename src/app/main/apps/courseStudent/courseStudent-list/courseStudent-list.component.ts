...
@Component({
  selector: "app-courseStudent-list",
  templateUrl: "./courseStudent-list.component.html",
  styleUrls: ["./courseStudent-list.component.scss"],
...
export class CourseStudentListComponent implements OnInit {
   ...
   * @param {CourseStudentListComponent} _courseStudentListService
   */
  constructor(
  ...
  async getRows() {
    await this._courseStudentListService
      .getDataTableRows(this.selectedCourseId)
      .then((response) => {
  ...
  addItem() {
    this.router.navigate(["/apps/course-student/list/new"], {
      queryParams: { id: this.selectedCourseId },
    });
  }

  editItem(row: any) {
    this.router.navigate([`/apps/course-student/list/new/${row.id}`], {
      queryParams: { id: this.selectedCourseId },
    });
  }
  ...
  deleteItem(id: string) {
    this.isLoading = true;
    this._courseStudentListService.deleteItem(id).then((response: any) => {
      this.isLoading = false;
      if (response.status) {
        Swal.fire({
          icon: "success",
