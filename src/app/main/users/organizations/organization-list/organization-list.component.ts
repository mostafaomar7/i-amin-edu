...
export class OrganizationListComponent implements OnInit {
   ...
   * @param {OrganizationListService} _organizationListService
   */
  constructor(
 ...
 */
  async getRows() {
    await this._organizationListService.getDataTableRows().then(response => {
      this.isLoading = false;
      if (response.status) {
        this.rows = response.innerData;
        this.activeCenters = this.rows.filter(row => (row.user.accountStatus == 2)).length
        this.allCenters = this.rows
 ...
 */
  addItem() {
    this.router.navigate([`/users/organizations/list/new`]);
  }

  viewItem(row: any) {
    // this.router.navigate([`/users/organizations/list/view/`]);
    this.router.navigate([`/users/organizations/list/view/${row.user.id}`]);

  }

  editItem(row: any) {
    this.router.navigate([`/users/organizations/list/new/${row.user.id}`]);
  }

  deleteItem(id: string) {
    this.isLoading = true;
    this._organizationListService.deleteItem(id).then((response: any) => {
      this.isLoading = false;
      if (response.status) {
        Swal.fire({
          icon: 'success',
