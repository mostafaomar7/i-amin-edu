...
export class AdminListComponent implements OnInit {
   ...
   * @param {AdminListService} _adminListService
   */
  constructor(
 ...
 */
  async getRows() {
    await this._adminListService.getDataTableRows().then(response => {
 ...
 */
  addItem() {
    this.router.navigate([`/users/admins/list/new`]);
  }

  editItem(row: any) {
    this.router.navigate([`/users/admins/list/new/${row.user.id}`]);
  }

  deleteItem(id: string) {
    this.isLoading = true;
    this._adminListService.deleteItem(id).then((response: any) => {
      this.isLoading = false;
      if (response.status) {
        Swal.fire({
          icon: 'success',
